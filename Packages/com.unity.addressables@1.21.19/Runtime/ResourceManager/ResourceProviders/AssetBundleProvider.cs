...
namespace UnityEngine.ResourceManagement.ResourceProviders
{
    ...
    public class AssetBundleResource : IAssetBundleResource, IUpdateReceiver
    {
        ...
        public static event Action<string> OnAssetBundleLoadStart;
        public static event Action<string> OnAssetBundleLoaded;
        public static event Action<string> OnAssetBundleUnloadStart;
        public static event Action<string> OnAssetBundleUnloaded;

        public void OnUnloadEventInvoke(string bundleName)
        {
            OnAssetBundleUnloaded?.Invoke(bundleName);
        }
        ...
        void AddCallbackInvokeIfDone(AsyncOperation operation, Action<AsyncOperation> callback)
        {
            ...
            {
                OnAssetBundleLoadStart?.Invoke(m_TransformedInternalId);
                operation.completed += op =>
                {
                    OnAssetBundleLoaded?.Invoke(m_TransformedInternalId);
                    callback(op);
                };
            }
        }
        ...
        private void WebRequestOperationCompleted(AsyncOperation op)
        {
            ...
            if (webReq != null && !UnityWebRequestUtilities.RequestHasErrors(webReq, out uwrResult))
            {
                if (!m_Completed)
                {
                    ...
                    AddBundleToProfiler(Profiling.ContentStatus.Active, m_Source);
#endif
                    // Because of bug https://forum.unity.com/threads/cant-load-from-downloaddependenciesasync-in-latest-package-update.1480392/
...
#endif
        {
            ...
            if (m_AssetBundle != null)
            {
                OnAssetBundleUnloadStart?.Invoke(m_TransformedInternalId);
            ...
            if (bundle != null)
            {
                ...
                if (bundle.Unload(out var unloadOp))
                {
                    ...
                    unloadOp.completed += op =>
                    {
                        ...
                        bundle.OnUnloadEventInvoke(location.InternalId);
                    };
                }
#else
                bundle.Unload();
